...
const API = {
  ...
  // ── Email Sending ──
  // /api/email requires an artist token (it sends as OTONAMI with a
  // caller-supplied Reply-To), so this goes through authFetch rather than a
  // bare fetch. The pitch-send flow always holds a token by this point — the
  // /api/pitches insert that precedes it already required one.
  ...
  // Currently unused (no caller), but it targets the same authenticated route,
  // so it carries the token too rather than being a broken path in waiting.
