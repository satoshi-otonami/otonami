...
export async function loadCurators() {
  try {
    const { data, error } = await supabase
      .from('curators')
      // Explicit column list, never select('*'). The anon key can read this
      // table, so the browser receives exactly what is listed here — and the
      // column-level GRANT for `anon` is scoped to this list, which makes a
      // stray '*' fail with 42501 rather than leak. pw_hash / payment_info /
      // verification_token / intro_mark_token are deliberately absent: nothing
      // artist-facing reads them. Every name below is verified against the live
      // table; `platform` / `audio_profile` / `preferred_tempo` appear in
      // mapCuratorFromDB but do NOT exist as columns, so listing them would
      ...
      .select(
        ...
        'pitches_received,pitches_responded,created_at'
      )
    ...
    // the SQL filter above by design — is_seed is in the column list above, so
    // this is a second guard on the same source of truth.
    ...
    return active.map(c => mapCuratorFromDB(c, stats[c.id]));
  } catch {
    return null;
  }
}
...
/**
 * Pitches for the signed-in artist.
 *
 * Keyed on artistId, never on the contact email: a label account's artists
 * share one address, so an artist_email filter would show one artist the
 * others' pitches. session_id is still OR-ed in to keep pitches from a
 * pre-login session attached to the browser that sent them.
 */
export async function loadPitches(artistId) {
  const sessionId = getSessionId();
  if (!sessionId && !artistId) return [];
  try {
    // artist_tracks(title) は pitches.track_id の埋め込み。トラッキングの
    // 楽曲別グループ表示に使う（pitches に曲名カラムは存在しない）。
    let query = supabase.from('pitches').select('*, artist_tracks(title)');
    if (sessionId && artistId) {
      query = query.or(`session_id.eq.${sessionId},artist_id.eq.${artistId}`);
    } else if (sessionId) {
    ...
    const { data, error } = await query
      ...
      .limit(300);
    if (error) throw error;
    // 重複排除（同じIDのピッチが session_id と artist_id の両方でヒットする場合）
    const seen = new Set();
...
function mapCuratorFromDB(row, stat) {
  return {
    ...
    isSeed: row.is_seed,
    // Curator-controlled intake switch. true = not accepting new pitches.
    ...
    // lib/match-score.js reads this camelCase key; leaving it out of this
    // mapper (the row itself carries it — loadCurators selects the column) made the
    // flag invisible to every score /studio computes. A curator with no
    // genres but open_to_all_genres=true scored null "マッチデータ不足" and
    // sank below zero-scoring curators in the list, which is the opposite of
    // what the setting means.
    openToAllGenres: row.open_to_all_genres === true,
