...
export async function loadCurators() {
  try {
    ...
    // the SQL filter above by design — select('*') carries is_seed, so this is a
    // second guard on the same source of truth.
...
function mapCuratorFromDB(row, stat) {
  return {
    ...
    // mapper (the row itself carries it — loadCurators selects '*') made the
