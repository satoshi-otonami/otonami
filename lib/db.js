...
export async function pickUnintroducedCurator() {
  ...
  const { data, error } = await db
    .from('curators')
    .select('id, name, playlist, url, playlist_url, region, type, opportunities, accepts, tier')
  ...
  if (!data) return null;
  // 列の意味を生成側に正しく渡すため正規化（accepts=ジャンル / opportunities=サービス /
  // type≠curator_type / tier=クレジットコスト / playlist=媒体名）。
  return {
    id: data.id,
    name: data.name,
    ...
    accepts: data.accepts || [],            // ジャンル。空のことがある → 生成側は空なら言及しない
    tier: data.tier ?? null,
  };
}
