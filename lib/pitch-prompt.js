// lib/pitch-prompt.js
// ピッチ生成（AIモード）のプロンプト組み立て。app/api/pitch/route.js から
// そのまま切り出した純関数で、外部I/O（認証・レート制限・Supabase・Anthropic）は
// 一切持たない。
//
// なぜ分けたか: モデル移行（sonnet-4-6 → sonnet-5）の回帰比較で、テスト側が
// 本番と同一のプロンプトを組み立てられる必要があったため。route handler に
// 予約語以外の named export を足さずに済むよう lib に置いている。
//
// 入力はすべて呼び出し側が用意する。DB から引く bio / hot_news / influences も
// 引数で受け取るだけで、ここでは取りに行かない。

import { describeTrackCharacteristics } from '@/lib/track-description';
import { FORBIDDEN_TONE_WORDS } from '@/lib/pitch-tone';
...
export function buildPitchPrompt({
  ...
  trackFeatures,
  artistDbBio = null,
  artistDbHotNews = null,
  artistDbInfluences = null,
}) {
...
${toneOverride ? `
═══ FINAL CHECK BEFORE YOU OUTPUT (do this silently, then output only the result) ═══
Re-read your own pitch body and EPK bio once, word by word, and verify BOTH:

1. NONE of these words appears anywhere, in any form or casing — not in the subject line, not in the pitch, not in the EPK, not softened as "not quite X", "less X than", "X undertones", "shades of X":
   ${FORBIDDEN_TONE_WORDS.join(', ')}
   If you find one, rewrite that sentence using the REQUIRED vocabulary from TONE LOCK before you output. Do not explain the fix — just output the corrected text.

2. The emotional character you described matches what the artist wrote in their Description. You were given NO information about the recipient, so there is nothing to tune the tone toward. If a sentence reads like it is reaching for a particular listener's taste, delete that reach and describe the song as the artist described it.
