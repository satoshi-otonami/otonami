...
// ── Genre match: recall-based (how much of the track's genres does the curator cover) ──
// curator.genres = ["Jazz", "Fusion", "City Pop"]
// artist.genres  = ["Jazz", "Funk", "Latin"]
// recall = |intersection| / |trackGenres| — rewards curators who explicitly cover the artist's genres
export function genreMatch(curatorGenres, artistGenres) {
  if (!curatorGenres?.length || !artistGenres?.length) return 0;
  const setC = new Set(curatorGenres.map(s => s.toLowerCase().trim()));
  let hits = 0;
  for (const g of artistGenres) {
    if (setC.has(g.toLowerCase().trim())) hits++;
  }
  return hits / artistGenres.length;
}
