...
**山下さん決定（2026-09-16）**: 配信日が近く、デプロイ完了を待たせられないため
「先に `+beni` で登録 → 後で紐付け替え」の案内を維持する。
ただし**デプロイが西村さんの登録より先に完了した場合は、新UIからの追加（credits=0）に切り替えてよい**。

---

## ケース A（推奨・デプロイが先に間に合った場合）— 手動SQL不要
