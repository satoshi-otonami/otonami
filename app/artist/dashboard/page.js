      ...
      <header style={{
        ...
        <div style={{ position: 'relative' }} ref={headerMenuRef}>
          ...
          >
            ...
            {artist.is_founding && (
              <span className="header-founding" title={`Founding Artist #${artist.founding_number}`} style={{
                padding: '2px 7px', borderRadius: 9999, fontSize: 9, fontWeight: 700,
                background: `linear-gradient(135deg, ${THEME.gold}, ${THEME.goldDark})`,
                color: '#fff', letterSpacing: '0.05em', fontFamily: THEME.font,
              }}>
                ◆ #{artist.founding_number}
              </span>
            )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                {artist.is_founding && (
                  <span style={{
                    ...
                    background: `linear-gradient(135deg, ${THEME.gold}, ${THEME.goldDark})`,
                    color: '#fff', letterSpacing: '0.05em', fontFamily: THEME.font,
                    boxShadow: '0 2px 8px rgba(196,149,106,0.30)',
                  }}>
                    ◆ FOUNDING #{artist.founding_number}
                  </span>
                )}
      ...
      >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8 }}>
          {MOOD_OPTIONS.map(m => {
            ...
            return (
              <button key={m} disabled={maxed} onClick={() => toggleArray('moods', m, 5)} style={{
                padding: '5px 12px', borderRadius: 100, fontSize: 11, fontWeight: 500,
                border: `1.5px solid ${sel ? THEME.gold : THEME.border}`,
                background: sel ? THEME.gold : THEME.card, color: sel ? '#fff' : THEME.text,
                cursor: maxed ? 'not-allowed' : 'pointer', fontFamily: THEME.font,
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8 }}>
            {form.moods.map(m => (
              <span key={m} style={{
                ...
                {m}
                <button type="button" aria-label={`${m} を外す`} onClick={() => toggleArray('moods', m, 5)}
                  style={{ background: 'none', border: 'none', color: THEME.gold, cursor: 'pointer', padding: '0 2px', fontSize: 13, lineHeight: 1, fontFamily: THEME.font }}
                >×</button>
              </span>
            ))}
