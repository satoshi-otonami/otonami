...
// Only positive, artist-owned metrics are surfaced publicly: curators reached
// and countries. Response rate was removed because it drops on curator
// non-reply (an external factor), under-representing the artist on a page
// meant as a credibility card.
async function getPitchStatsForArtist(artistId) {
  const db = getServiceSupabase();
  const { data: pitches, error } = await db
    .from('pitches')
    .select('curator_id, curators(region, is_seed)')
    .eq('artist_id', artistId)
    .neq('status', 'draft');

  if (error || !pitches || pitches.length === 0) {
    return { curators_reached: 0, countries: 0 };
  }

  // Pitches sent to seed/staff/test curators are real rows (they carry credits
  // and earnings) but must not inflate a public badge. A pitch whose curator
  // row is missing keeps counting, as it did before.
  const real = pitches.filter((p) => !isSeedCurator(p.curators));

  const curators = new Set(real.map((p) => p.curator_id).filter(Boolean));
  const regions = new Set(real.map((p) => p.curators?.region).filter(Boolean));
