...
// 画像は Supabase storage か自サイト配下（public/）のみ。Spotify 等の外部画像は使わない。
// 写真の無いカードはイニシャル丸で代替表示される。
...
export const CURATOR_CARDS = [
  ...
  },
  {
    ...
    // 2026-09-24 本人から文言・引用の両方の許諾を受領
    published: true,
    name: 'Van Paugam',
    descriptor: { ja: 'NEO-CITY POP キュレーター', en: 'NEO-CITY POP curator' },
    statement: {
    ...
    // curators.icon_url の縮小コピー（自サイト配下）
    photo: { src: '/images/lp-cases/van-paugam.jpg', width: 192, height: 192 },
    link: 'https://open.spotify.com/playlist/6US7ZVtZ163U0ylpB0vBdb',
    linkLabel: 'NEO-CITY POP',
  },
];
...
export const STORIES = [
  {
    ...
    tracks: ['C1 (Tokyo Inner Loop)'],
    // キュレーター名そのものを局サイトへのリンクにする
    curatorLink: 'https://radioarmazem.net/',
    // 画像は自サイト配下の縮小コピー（元: EPK ヒーロー = artists.cover_url / curators.icon_url）
    artistPhoto: { src: '/images/lp-cases/route14band.jpg', width: 960, height: 640 },
    curatorIcon: { src: '/images/lp-cases/radio-armazem.jpg', width: 160, height: 160 },
    body: {
...
export const ARTIST_CARDS = [
  {
    ...
    // EPK 未作成のためプロフィールのカバー（artists.cover_url）の縮小コピー
    photo: { src: '/images/lp-cases/epic-vanguard.jpg', width: 960, height: 411 },
    result: {
  ...
  },
  {
    ...
    // EPK ヒーロー（artists.cover_url）の縮小コピー
    photo: { src: '/images/lp-cases/chihitek.jpg', width: 960, height: 540 },
    result: {
      ja: 'シティポップ専門の海外キュレーターのプレイリストに掲載',
      en: 'Added to a playlist by an international curator who specializes in city pop',
    },
    // 2026-09-24 Van Paugam 氏の許諾受領 → 実名版を公開
    resultNamed: {
      published: true,
...
/**
 * Only what may be shown publicly. Call this on the server (app/page.js) and pass the result
 * down as props: importing this file from a client component would ship the unpublished
 * cards (pending quotes, names) inside the public JS bundle even though they never render.
 */
export function getPublishedLpCases() {
  return {
    intro: LP_CASES_INTRO,
    guarantee: GUARANTEE_NOTE,
    // Anonymous cases minus the ones already told under a published real name.
    anon: ANON_CASES.filter((c) => !(c.supersededBy && PUBLISHED_IDS.has(c.supersededBy))),
    curators: CURATOR_CARDS.filter((c) => c.published).map((c) => ({
      ...c,
      quote: c.quote?.published ? c.quote : null,
    })),
    stories: STORIES.filter((c) => c.published),
    artists: ARTIST_CARDS.filter((c) => c.published).map((c) => ({
      ...c,
      resultNamed: c.resultNamed?.published ? c.resultNamed : null,
    })),
  };
}
