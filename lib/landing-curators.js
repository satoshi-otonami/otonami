...
export async function getLandingCurators() {
  try {
    ...
    const { data, error } = await supabase
      .from('curators')
      .select(`name, playlist, region, type, icon_url, created_at, tier, ${PUBLIC_CURATOR_COLUMNS}`)
      .or('is_seed.is.null,is_seed.eq.false')
      .order('created_at', { ascending: false });
    if (error) throw new Error(error.message);

    const rows = (data || []).filter(isPublicCurator);
    ...
    // Cheapest pitch on the roster, in credits. The landing page prints a
    // "from ¥X" figure and must never hard-code it: the floor is whatever the
    // lowest tier among publicly listed curators happens to be (2 today, but a
    // tier-1 signup would silently make a literal wrong). Same rule as the
    // headcount — read it, never type it.
    const tiers = rows.map(c => c.tier).filter(t => Number.isFinite(t) && t > 0);
