  ...
  const handleResendVerification = async () => {
    ...
    try {
      const res = await fetch('/api/resend-verification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, type: 'artist' }),
      });
  ...
  const submitOtp = async (code) => {
    ...
    try {
      const res = await fetch('/api/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, otp_code: code, type: 'artist' }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (data.attemptsRemaining !== undefined) {
          if (data.attemptsRemaining === 0) {
            setOtpError('試行回数を超えました。しばらく経ってから再度お試しください。');
          } else {
            setOtpError(`コードが正しくありません。残り${data.attemptsRemaining}回`);
          }
        } else {
                  ...
                  {emailNotVerified && (
                    <div style={{ marginTop: 12 }}>
                      <button onClick={handleResendVerification} disabled={resendVerifyStatus === 'loading'}
                        ...
                        {resendVerifyStatus === 'loading' ? '送信中...' : '認証メールを再送信'}
                      </button>
                      {resendVerifyStatus === 'sent' && <p style={{ color: THEME.green, fontSize: 12, marginTop: 8 }}>認証メールを送信しました</p>}
                    </div>
                  )}
                </div>
              )}
            ...
            <>
              ...
              <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 12 }}>
                <button onClick={handleResendOtp} disabled={resendCooldown > 0}
                  ...
                  {resendCooldown > 0 ? `認証コードを再送信（${resendCooldown}秒）` : '認証コードを再送信'}
                </button>
