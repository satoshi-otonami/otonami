...
const API = {
  ...
  // ── AI Pitch Generation ──
  async generatePitch(artist, curator, style, links, followers, userName) {
    const res = await fetch('/api/pitch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ artist, curator, style, links, followers, userName }),
    });
    const data = await res.json();
  ...
  // ── Email Sending ──
  async sendPitchEmail(pitchId, toEmail, toName, pitchText, epk, artistName, trackUrl) {
