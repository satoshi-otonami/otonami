...
import { isTestCurator } from './curator-visibility';
...
export async function loadCurators() {
  try {
    ...
    if (error) throw error;
    // Test/dummy curators are excluded from artist-facing lists entirely
    // (they used to slip through the is_seed filter and could receive real pitches).
    const visible = data.filter(c => !isTestCurator(c));
...
//       かつ既存の公開条件 is_seed <> true、Testキュレーター除外（isTestCurator）。
//       並びは created_at DESC、先頭の非Test 1件を採用。
...
export async function pickUnintroducedCurator() {
  ...
  // Test/dummy curators must never be introduced publicly on SNS; skip to the
  // next candidate (fetch a few rows since the JS predicate can't run in SQL).
  const data = (rows || []).find(c => !isTestCurator(c)) || null;
  if (!data) return null;
